...
def build_xpos_cache(slen, angle, decay, recurrent_chunk_size,
                     activate_recurrent=False, chunkwise_recurrent=False):
    if activate_recurrent:
        ...
        block_index = torch.arange(recurrent_chunk_size).to(decay)
        mask = torch.tril(torch.ones(recurrent_chunk_size, recurrent_chunk_size).to(decay))
        mask = torch.masked_fill(block_index[:, None] - block_index[None, :], ~mask.bool(), float("inf"))
        ...
        cross_decay = torch.exp(decay * recurrent_chunk_size)
        inner_decay = torch.exp(decay[:, None] * (block_index + 1))
        cross_decay = cross_decay[:, None, None]
        inner_decay = inner_decay[:, :, None] / (scale / scale[:, -1, None])
        retention_rel_pos = ((sin, cos), (mask, cross_decay, inner_decay))
    else:
...
class RetNetDecoder(nn.Module):
    ...
    def build_xpos_cache(self, slen, angle, decay, recurrent_chunk_size, activate_recurrent, chunkwise_recurrent):
        return build_xpos_cache(slen, angle, decay, recurrent_chunk_size,
                                activate_recurrent, chunkwise_recurrent)
    ...
    def forward(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, l_aux_i = layer(
                    ...
                    retention_rel_pos=self.xpos_cache,      #retention_rel_pos,
                    chunkwise_recurrent=self.chunkwise_recurrent,
            )
